// frontend/src/components/hr/ArchivedCVsTable.jsx
...
function ArchivedCVsTable({
  data = [],
  onViewDetail,
  onDeleteCV, // Menerima prop untuk hapus individual
  onDownloadCV,
  sortColumn,
  sortDirection,
  onSort,
  isSelectMode, // Menerima prop mode seleksi
  selectedCVs = [],
  setSelectedCVs,
  firstItemIndex = 0,
}) {
  ...
  const handleCheckAllChange = (event) => {
    if (!setSelectedCVs) return;
    // Di halaman arsip, semua yang tampil bisa dipilih
    const idsOnPage = data.map((cv) => cv.id);
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {data && data.length > 0 ? (
                  ...
                  <tr key={cv.id} className={`transition duration-150 ease-in-out ${selectedCVs.includes(cv.id) ? 'bg-indigo-50' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                      <div className="flex items-center justify-center space-x-3">
                        ...
                        {/* Tombol Hapus Individual (HANYA UNTUK ARSIP) */}
                        {onDeleteCV && (
                           <button onClick={() => onDeleteCV(cv)} className="text-gray-500 hover:text-red-600 p-1 rounded hover:bg-red-50" title="Hapus CV Permanen">
              ...
              <tr>
                <td colSpan={isSelectMode ? 11 : 10} className="px-6 py-8 whitespace-nowrap text-center text-sm text-gray-500">
...
export default ArchivedCVsTable;