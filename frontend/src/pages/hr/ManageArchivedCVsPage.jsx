...
const API_UNARCHIVE_URL = `${API_BASE_URL}/api/hr/applications/unarchive`;
const API_BULK_DELETE_URL = `${API_BASE_URL}/api/hr/applications/bulk-delete`;
...
function ManageArchivedCVsPage() {
  ...
  const fetchArchivedCvData = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(API_GET_ARCHIVED_CVS_URL, { headers: { Authorization: `Bearer ${token}` } });
      setAllCvData(Array.isArray(response.data) ? response.data : []);
    } catch (err) {
      console.error("Error fetching archived CV data:", err); setError(err);
      Notiflix.Report.failure("Load Data Failed", err.response?.data?.message || err.message || "An error occurred.", "Okay");
    } finally { setLoading(false); }
  }, []);

  useEffect(() => { fetchArchivedCvData(); }, [fetchArchivedCvData]);
  ...
  const processedCvData = useMemo(() => {
    ...
    if (searchTerm) { const l = searchTerm.toLowerCase(); filtered = filtered.filter(cv => (cv.fullName?.toLowerCase().includes(l)) || (cv.qualification?.toLowerCase().includes(l)) || (cv.email?.toLowerCase().includes(l))); }
    ...
    if (dateFilter instanceof Date && !isNaN(dateFilter.getTime())) { const f = new Date(dateFilter).setHours(0,0,0,0); filtered = filtered.filter(cv => { try { const d = new Date(cv.createdAt).setHours(0,0,0,0); return !isNaN(d) && d === f; } catch { return false; }}); }
    
    const filteredAndSortedData = [...filtered];
    if (sortColumn) {
      filteredAndSortedData.sort((a, b) => {
        let aV = a[sortColumn]; let bV = b[sortColumn]; if (sortColumn === 'final_recommendation') { aV = a.qualitative_assessment?.final_recommendation || null; bV = b.qualitative_assessment?.final_recommendation || null; }
        const isAsc = sortDirection === "asc"; if (aV == null && bV == null) return 0; if (aV == null) return isAsc ? -1 : 1; if (bV == null) return isAsc ? 1 : -1;
        if (sortColumn === "createdAt") { const dA = new Date(aV).getTime(); const dB = new Date(bV).getTime(); if (isNaN(dA) && isNaN(dB)) return 0; if (isNaN(dA)) return isAsc ? -1 : 1; if (isNaN(dB)) return isAsc ? 1 : -1; return isAsc ? dA - dB : dB - dA; }
        else if (sortColumn === 'similarity_score') { const nA = Number(aV); const nB = Number(bV); if (isNaN(nA) && isNaN(nB)) return 0; if (isNaN(nA)) return isAsc ? 1 : -1; if (isNaN(nB)) return isAsc ? -1 : 1; return isAsc ? nA - nB : nB - nA; }
        else if (sortColumn === 'passed_hard_gate') { const vA = aV === true ? 1 : 0; const vB = bV === true ? 1 : 0; return isAsc ? vA - vB : vB - vA; }
        else if (typeof aV === "string" && typeof bV === "string") { return isAsc ? aV.localeCompare(bV) : bV.localeCompare(aV); }
        else { return isAsc ? (aV < bV ? -1 : aV > bV ? 1 : 0) : (bV < aV ? -1 : bV > aV ? 1 : 0); }
      });
    }
    ...
    return { data: paginatedData, filteredAndSortedData, totalItems, totalPages, currentPage: validPage, firstItemIndex: totalItems > 0 ? startIndex + 1 : 0, lastItemIndex: Math.min(startIndex + ITEMS_PER_PAGE, totalItems) };
  }, [allCvData, searchTerm, statusFilter, qualificationFilter, dateFilter, sortColumn, sortDirection, currentPage]);

  useEffect(() => { setCurrentPage(1); if (!isSelectMode) setSelectedCVs([]); }, [searchTerm, statusFilter, qualificationFilter, dateFilter, sortColumn, sortDirection, isSelectMode]);
  useEffect(() => { const { totalPages, currentPage } = processedCvData; if (currentPage > totalPages && totalPages > 0) { setCurrentPage(totalPages); } }, [processedCvData.totalItems, processedCvData.totalPages, currentPage]);
  ...
  const handleSort = (column) => { setSortDirection(prev => (sortColumn === column && prev === 'asc' ? 'desc' : 'asc')); setSortColumn(column); };
  const handlePageChange = (page) => { if (page >= 1 && page <= processedCvData.totalPages) { setCurrentPage(page); } };
  ...
  const handleDeleteCV = useCallback((cv) => {
    Notiflix.Confirm.show(
      ...
      'Cancel',
      async () => {
        const deleteApiUrl = API_DELETE_URL(cv.id); // Asumsi endpoint delete sama
        try {
          ...
          fetchArchivedCvData(); // Refresh data arsip
        } catch (error) {
          Notiflix.Loading.remove();
  ...
  }, [fetchArchivedCvData]); // Dependency
  ...
  const handleUnarchiveSelected = useCallback(async () => {
    if (selectedCVs.length === 0) { Notiflix.Report.warning("No Items Selected", "Select items to unarchive.", "Okay"); return; }
    Notiflix.Confirm.show('Confirm Unarchive', `Unarchive ${selectedCVs.length} selected applications?`, 'Unarchive', 'Cancel', async () => {
      try {
        Notiflix.Loading.standard('Unarchiving...'); const token = getAuthToken(); if (!token) throw new Error("Missing token");
        await axios.put(API_UNARCHIVE_URL, { ids: selectedCVs }, { headers: { Authorization: `Bearer ${token}` } });
        Notiflix.Loading.remove(); Notiflix.Report.success('Unarchived', `${selectedCVs.length} applications unarchived.`, 'Okay');
        await fetchArchivedCvData(); setSelectedCVs([]); setIsSelectMode(false);
      } catch (error) { Notiflix.Loading.remove(); console.error('Error unarchiving:', error); Notiflix.Report.failure('Unarchive Failed', error.response?.data?.message || "Error", 'Okay'); }
    }, () => {});
  ...
  const handleBulkDeleteArchived = useCallback(async () => {
    ...
    Notiflix.Confirm.show('Confirm Permanent Delete', `PERMANENTLY DELETE ${selectedCVs.length} selected applications? This cannot be undone.`, 'DELETE', 'Cancel', async () => {
      try {
        Notiflix.Loading.standard('Deleting...'); const token = getAuthToken(); if (!token) throw new Error("Missing token");
        await axios.post(API_BULK_DELETE_URL, { ids: selectedCVs }, { headers: { Authorization: `Bearer ${token}` } });
        Notiflix.Loading.remove(); Notiflix.Report.success('Deleted', `${selectedCVs.length} applications deleted.`, 'Okay');
        await fetchArchivedCvData(); setSelectedCVs([]); setIsSelectMode(false);
      } catch (error) { Notiflix.Loading.remove(); console.error('Error bulk deleting archived:', error); Notiflix.Report.failure('Delete Failed', error.response?.data?.message || "Error", 'Okay'); }
    }, () => {}, { okButtonBackground: '#EF4444', titleColor: '#DC2626' });
  }, [selectedCVs, fetchArchivedCvData]);
  ...
  const handleBulkDownloadSelected = useCallback(async () => {
    ...
    Notiflix.Loading.standard(`Preparing ${selectedCVs.length} CVs...`); const token = getAuthToken(); if (!token) { Notiflix.Loading.remove(); return; }
    ...
    try {
      ...
      if (filesAdded > 0) {
        ...
        zip.generateAsync({ type: "blob", compression: "DEFLATE", compressionOptions: { level: 6 } })
          .then(content => { saveAs(content, `selected_archived_cvs_${new Date().toISOString().split('T')[0]}.zip`); Notiflix.Loading.remove(); })
  ...
  if (loading && allCvData.length === 0) return <div className="text-center p-8 text-gray-600">Memuat data arsip...</div>;
  ...
  return (
    <div className="manage-cv-page-container p-4">
      <h2 className="text-2xl font-semibold text-gray-800 mb-16">Archived CV Applications</h2>

      <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          <input type="text" placeholder="Search..." value={searchTerm} onChange={handleSearchChange} className="shadow-sm border border-gray-300 rounded py-2 px-3 text-sm focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-red-500 flex-grow md:flex-grow-0 md:w-48"/>
          ...
          <div className="relative w-full sm:w-auto min-w-[150px]">
            <DatePicker selected={dateFilter} onChange={handleDateFilterChange} dateFormat="dd/MM/yyyy" placeholderText="dd/mm/yyyy" isClearable className="shadow-sm border border-gray-300 rounded w-full py-2 px-3 text-sm focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-red-500 pr-8"/>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <button onClick={() => navigate('/admin/manage-cv')} className="flex items-center px-3 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-1 bg-gray-500 hover:bg-gray-600 focus:ring-gray-400 text-white text-sm whitespace-nowrap">
      ...
      {isSelectMode && (
        <div className="mb-4 p-3 bg-gray-100 rounded border flex flex-col sm:flex-row justify-between items-center gap-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={handleSelectAll} className="px-2 py-1 text-xs border rounded bg-white hover:bg-gray-100 focus:outline-none focus:ring-1 focus:ring-blue-500">All ({processedCvData.filteredAndSortedData.length})</button>
            <button onClick={handleSelectNone} disabled={selectedCVs.length === 0} className="px-2 py-1 text-xs border rounded bg-white hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-1 focus:ring-blue-500">None</button>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {selectedCVs.length > 0 ? (
              <>
                <button onClick={handleUnarchiveSelected} title="Unarchive selected" className="flex items-center px-3 py-1.5 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-1 bg-green-600 hover:bg-green-700 focus:ring-green-500 text-white text-xs whitespace-nowrap">
                  <ArchiveBoxIcon className="mr-1 h-4 w-4" /> Unarchive ({selectedCVs.length})
                </button>
                <button onClick={handleBulkDeleteArchived} title="Delete selected permanently" className="flex items-center px-3 py-1.5 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-1 bg-red-600 hover:bg-red-700 focus:ring-red-500 text-white text-xs whitespace-nowrap">
                  <TrashIcon className="mr-1 h-4 w-4" /> Delete ({selectedCVs.length})
                </button>
                <button onClick={handleBulkDownloadSelected} title="Download selected CVs as ZIP" className="flex items-center px-3 py-1.5 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-1 bg-blue-600 hover:bg-blue-700 focus:ring-blue-500 text-white text-xs whitespace-nowrap">
      ...
      {!loading && allCvData.length === 0 && !error && ( <div className="text-center text-gray-500 my-8 py-4">No archived applications found.</div> )}
      ...
      {(processedCvData.data.length > 0 || (loading && allCvData.length > 0)) && (
        <ArchivedCVsTable
          ...
          onDeleteCV={handleDeleteCV} // <-- Teruskan handler hapus individual
...
export default ManageArchivedCVsPage;