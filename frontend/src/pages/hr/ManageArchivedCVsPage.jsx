...
const API_VIEW_URL = (id) =>
  `${API_BASE_URL}/api/hr/applications/${id}/view-cv`;
const API_DOWNLOAD_URL = (id) =>
  `${API_BASE_URL}/api/hr/applications/${id}/download-cv`;
...
function ManageArchivedCVsPage() {
  ...
  const availableQualifications = useMemo(() => {
    const qualifications = (allCvData || [])
  ...
  const processedCvData = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(
        (cv) =>
          cv.fullName?.toLowerCase().includes(l) ||
          cv.qualification?.toLowerCase().includes(l) ||
          cv.email?.toLowerCase().includes(l)
      );
    }
    if (statusFilter)
    ...
    if (qualificationFilter)
      filtered = filtered.filter(
        (cv) => cv.qualification === qualificationFilter
      );
  ...
  useEffect(() => {
    setCurrentPage(1);
    if (!isSelectMode) setSelectedCVs([]);
  }, [
    searchTerm,
    statusFilter,
    qualificationFilter,
        ...
        async () => {
          ...
          try {
            ...
            Notiflix.Report.success(
              "Deleted",
              `Application from ${cv.fullName} deleted.`,
              "Okay"
            );
  ...
  const handleSelectAll = useCallback(
    () =>
      setSelectedCVs(processedCvData.filteredAndSortedData.map((cv) => cv.id)),
    [processedCvData.filteredAndSortedData]
  );
  const handleSelectNone = useCallback(() => setSelectedCVs([]), []);
  const handleSelectAccepted = useCallback(
    () =>
      setSelectedCVs(
        processedCvData.filteredAndSortedData
  ...
  const handleSelectRejected = useCallback(
    () =>
      setSelectedCVs(
        processedCvData.filteredAndSortedData
    ...
    Notiflix.Confirm.show(
      ...
      async () => {
        try {
          ...
          Notiflix.Report.success(
            "Unarchived",
            `${selectedCVs.length} applications unarchived.`,
            "Okay"
          );
    ...
    Notiflix.Confirm.show(
      ...
      async () => {
        try {
          ...
          Notiflix.Report.success(
            "Deleted",
            `${selectedCVs.length} applications deleted.`,
            "Okay"
          );
          await fetchArchivedCvData();
          setSelectedCVs([]);
          ...
          Notiflix.Loading.remove();
          console.error("Error bulk deleting archived:", error);
          Notiflix.Report.failure(
            "Delete Failed",
            error.response?.data?.message || "Error",
            "Okay"
  ...
  const handleBulkDownloadSelected = useCallback(async () => {
    ...
    try {
      const downloadPromises = selectedCVs.map(async (cvId) => {
        ...
        if (cv?.cvFileObjectKey) {
          try {
            const response = await axios.get(API_DOWNLOAD_URL(cv.id), {
              responseType: "blob",
              headers: { Authorization: `Bearer ${token}` },
            });
            const safeFullName = (cv.fullName || `app_${cv.id}`).replace(
              /[^a-z0-9]/gi,
              "_"
            );
            const safeQual = (cv.qualification || "pos").replace(
              /[^a-z0-9]/gi,
              "_"
            );
            const extension =
              cv.cvFileName?.split(".").pop()?.toLowerCase() || "pdf";
            zip.file(
              `${cv.id}_${safeFullName}_${safeQual}.${extension}`,
              response.data
            );
            filesAdded++;
          } catch (downloadError) {
  ...
  if (loading && allCvData.length === 0)
    return (
      <div className="text-center p-8 text-gray-600">Memuat data arsip...</div>
    );
  if (error && allCvData.length === 0)
    return (
  ...
  return (
    <div className="manage-cv-page-container p-4">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          ...
          <div className="relative w-full sm:w-auto">
            <select
              ...
              className="shadow-sm border border-gray-300 rounded w-full py-2 px-3 text-sm focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-red-500 pr-8 appearance-none"
            >
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          >
            {isSelectMode ? (
              ...
              <>
                {" "}
                <CheckCircleIcon className="mr-1.5 h-4 w-4" /> Select for Bulk
                Actions{" "}
              </>
            )}
          </button>
          <button
            onClick={() => navigate("/admin/manage-cv")}
        ...
        <div className="mb-4 p-3 bg-gray-100 rounded border flex flex-col sm:flex-row justify-between items-center gap-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="px-2 py-1 text-xs border rounded bg-white hover:bg-gray-100 focus:outline-none focus:ring-1 focus:ring-blue-500"
            >
              All ({processedCvData.filteredAndSortedData.length})
            </button>
            <button
              onClick={handleSelectNone}
              disabled={selectedCVs.length === 0}
              className="px-2 py-1 text-xs border rounded bg-white hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-1 focus:ring-blue-500"
            >
              None
            </button>
            <button
              onClick={handleSelectAccepted}
              className="px-2 py-1 text-xs border rounded bg-white hover:bg-green-50 focus:outline-none focus:ring-1 focus:ring-green-500 text-green-700"
            >
              Accepted
            </button>
            <button
              onClick={handleSelectRejected}
              className="px-2 py-1 text-xs border rounded bg-white hover:bg-red-50 focus:outline-none focus:ring-1 focus:ring-red-500 text-red-700"
            >
              Rejected
            </button>
          </div>
            ...
            {selectedCVs.length > 0 ? (
              <>
                <button
                  ...
                  className="flex items-center px-3 py-1.5 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-1 bg-green-600 hover:bg-green-700 focus:ring-green-500 text-white text-xs whitespace-nowrap"
                >
                  <ArchiveBoxIcon className="mr-1 h-4 w-4" /> Unarchive (
                  {selectedCVs.length})
                </button>
                <button
                ...
                <button
                  ...
                  title="Download selected CVs as ZIP"
                  className="flex items-center px-3 py-1.5 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-1 bg-blue-600 hover:bg-blue-700 focus:ring-blue-500 text-white text-xs whitespace-nowrap"
                >
                  <ArrowDownTrayIcon className="mr-1 h-4 w-4" /> Download (
                  {selectedCVs.length})
                </button>
              </>
            ) : (
              <span className="text-sm text-gray-500 italic">
                Select CVs to perform bulk actions.
              </span>
            )}
          </div>
        </div>
      ...
      {loading && allCvData.length > 0 && (
        <div className="text-center text-gray-600 mb-4 py-4">
          Updating list...
        </div>
      )}
      {!loading &&
        processedCvData.data.length === 0 &&
        allCvData.length > 0 && (
          <div className="text-center text-gray-500 my-8 py-4">
            No CVs match the current filters.
          </div>
        )}
      {!loading && allCvData.length === 0 && !error && (
        <div className="text-center text-gray-500 my-8 py-4">
      ...
      {!loading && processedCvData.totalItems > ITEMS_PER_PAGE && (
        <div className="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6 mt-0 -mt-px rounded-b-lg shadow-md">
          <div className="flex flex-1 justify-between sm:hidden">
            <button
              onClick={() => handlePageChange(processedCvData.currentPage - 1)}
              disabled={processedCvData.currentPage <= 1}
            ...
            <span className="text-sm text-gray-700">
              Page {processedCvData.currentPage}/{processedCvData.totalPages}
            </span>
            <button
              onClick={() => handlePageChange(processedCvData.currentPage + 1)}
              disabled={
                processedCvData.currentPage >= processedCvData.totalPages
              }
          ...
          <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-gray-700">
                ...
                <span className="font-medium">
                  {processedCvData.firstItemIndex}
                </span>{" "}
                to{" "}
                <span className="font-medium">
                  {processedCvData.lastItemIndex}
                </span>{" "}
                of{" "}
                <span className="font-medium">
                  {processedCvData.totalItems}
                </span>{" "}
                results{" "}
              </p>{" "}
            </div>
            <div>
              ...
              >
                <button
                  onClick={() =>
                    handlePageChange(processedCvData.currentPage - 1)
                  }
                  disabled={processedCvData.currentPage <= 1}
                ...
                <span className="relative inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-700 ring-1 ring-inset ring-gray-300">
                  {" "}
                  Page {processedCvData.currentPage} of{" "}
                  {processedCvData.totalPages}{" "}
                </span>
                <button
                  onClick={() =>
                    handlePageChange(processedCvData.currentPage + 1)
